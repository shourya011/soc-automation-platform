    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          <Activity className="w-16 h-16 text-light-blue dark:text-cyber-blue animate-spin mx-auto mb-4" />
  ...
  return (
    <div className="space-y-6 animate-slide-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-orbitron font-bold text-gray-900 dark:text-white mb-2">Security Operations Center</h1>
          <p className="text-gray-600 dark:text-gray-400">Real-time monitoring and automation dashboard</p>
        </div>
        <div className="glass-card px-4 py-2">
          <div className="flex items-center">
            <div className="w-2 h-2 bg-cyber-green rounded-full mr-2 animate-pulse"></div>
            <span className="text-sm text-gray-700 dark:text-gray-300">Live Monitoring Active</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {kpiCards.map((kpi, idx) => (
          <div key={idx} className="glass-card-hover p-6 relative overflow-hidden">
            <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${kpi.bgColor} rounded-full blur-2xl opacity-30 dark:opacity-50 -mr-16 -mt-16`}></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-card p-6">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={data?.responseTimeChart || []}>
              ...
              <YAxis stroke="#6B7280" className="dark:stroke-gray-400" label={{ value: 'Minutes', angle: -90, position: 'insideLeft', fill: '#6B7280' }} />
              <Tooltip 
                contentStyle={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '8px' }}
                labelStyle={{ color: '#111827' }}
                wrapperClassName="dark:[&>div]:!bg-cyber-darker dark:[&>div]:!border-white/10"
              />
              <Line 
                type="monotone" 
        ...
        <div className="glass-card p-6">
          <h3 className="text-xl font-bold text-gray-900 mb-6 flex items-center">
            <AlertTriangle className="w-5 h-5 text-cyber-pink mr-2" />
            Alert Distribution
          </h3>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
      ...
      {/* Recent Alerts */}
      <div className="glass-card p-6">
        <h3 className="text-xl font-bold text-gray-900 mb-6">Recent Critical Alerts</h3>
        <div className="space-y-3">
          {(data?.recentAlerts || []).map((alert, idx) => (
            <div key={idx} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-all cursor-pointer border-l-4" style={{ borderColor: alert.severity === 'critical' ? '#FF6B9D' : alert.severity === 'high' ? '#A855F7' : '#00D9FF' }}>
              <div className="flex items-center space-x-4">
                <AlertTriangle className={`w-5 h-5 ${alert.severity === 'critical' ? 'text-cyber-pink' : 'text-cyber-blue'}`} />
                <div>
                  <div className="text-gray-900 font-semibold">{alert.id}</div>
                  <div className="text-sm text-gray-600">{alert.source}</div>
                </div>
              </div>
              <div className="flex items-center space-x-6">
                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                  alert.severity === 'critical' ? 'bg-cyber-pink/20 text-cyber-pink' :
