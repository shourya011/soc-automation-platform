  ...
  return (
    <div className="space-y-6 animate-slide-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-orbitron font-bold text-gray-900 dark:text-white mb-2">Alert Management</h1>
          <p className="text-gray-600 dark:text-gray-400">Monitor and respond to security alerts in real-time</p>
        </div>
        <div className="flex items-center space-x-2">
          <div className="glass-card px-4 py-2">
            <span className="text-sm text-gray-600 dark:text-gray-400">Active Alerts: </span>
            <span className="text-lg font-bold text-red-600 dark:text-cyber-red">{filteredAlerts.length}</span>
          </div>
        </div>
      </div>
      ...
      <div className="glass-card overflow-hidden">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-300 dark:border-white dark:border-opacity-10">
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">Alert ID</th>
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-white dark:divide-opacity-5">
              {filteredAlerts.map((alert) => (
                <tr key={alert.id} className="hover:bg-gray-100 dark:hover:bg-white dark:hover:bg-opacity-5 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <AlertTriangle className={`w-4 h-4 mr-2 ${getSeverityColor(alert.severity).split(' ')[0]}`} />
                      <span className="text-gray-900 dark:text-white font-mono text-sm">{alert.id}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-gray-700 dark:text-gray-300">{alert.type}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-gray-700 dark:text-gray-300">{alert.source}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-gray-700 dark:text-gray-300 font-mono text-sm">{alert.ip}</td>
        ...
        {filteredAlerts.length === 0 && !loading && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 dark:text-gray-400">No alerts found matching your filters</p>
          </div>
        )}
      </div>
    </div>
