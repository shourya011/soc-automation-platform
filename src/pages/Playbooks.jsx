  ...
  return (
    <div className="space-y-6 animate-slide-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-orbitron font-bold text-white mb-2">Playbook Automation</h1>
          <p className="text-gray-400">Automated response workflows for security incidents</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: 'Total Playbooks', value: playbooks.length, icon: Layers, color: 'text-cyber-blue' },
          { label: 'Executions Today', value: '127', icon: Play, color: 'text-cyber-green' },
          { label: 'Avg Success Rate', value: '96%', icon: CheckCircle, color: 'text-cyber-purple' },
          { label: 'Time Saved', value: '45h', icon: Clock, color: 'text-cyber-pink' },
        ].map((stat, idx) => (
          ...
          <div key={playbook.id} className="glass-card-hover overflow-hidden group">
            ...
            <div className="p-6">
              ...
              <div className="flex items-start justify-between mb-4">
                ...
                <span className="text-xs text-gray-400 font-mono">{playbook.id}</span>
              </div>
              ...
              {/* Category Tag */}
              <span className="inline-block px-3 py-1 bg-white bg-opacity-5 rounded-full text-xs text-gray-300 mb-4">
                {playbook.category}
              </span>

              {/* Stats */}
              <div className="grid grid-cols-3 gap-4 mb-6 py-4 border-t border-b border-white border-opacity-10">
                <div>
                  <div className="text-xs text-gray-400 mb-1">Success Rate</div>
                  <div className="text-lg font-bold text-cyber-green">{playbook.successRate}%</div>
                </div>
                <div>
                  <div className="text-xs text-gray-400 mb-1">Steps</div>
                  <div className="text-lg font-bold text-white">{playbook.steps}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-400 mb-1">Avg Time</div>
                  <div className="text-lg font-bold text-cyber-blue">{playbook.avgExecutionTime}</div>
                </div>
              </div>

              {/* Last Run */}
              <div className="text-xs text-gray-400 mb-4">
                Last run: <span className="text-white">{playbook.lastRun}</span>
              </div>
              ...
              <div className="flex space-x-2">
                ...
                <button className="px-4 py-2 bg-white bg-opacity-5 text-gray-300 rounded-lg hover:bg-opacity-10 transition-all">
